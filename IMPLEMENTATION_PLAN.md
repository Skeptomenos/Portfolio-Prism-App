...
| 1.2.4 | `src-tauri/python/portfolio_src/core/security_mapper.py` | Verify OPENFIGI_API_KEY usage - ensure proxied or remove | 15m | **DONE** (v0.1.7) - Removed unused file with direct API key violation |
...
| 3.5 | `src/lib/scrubber.ts` | Add missing PII patterns (credit cards, SSN, IP, file paths) + ReDoS protection | 30m | **DONE** (v0.3.4) |
...
| 5.1.7 | `src-tauri/python/portfolio_src/core/tr_bridge.py` | Validate Response ID matches Request ID (prevent protocol desync) | 15m | **DONE** (v0.5.4) |
...
| Task | File | Issue | Est. | Status |
|------|------|-------|------|--------|
| 6.1 | `.env.example` | Document Echo Bridge token requirement + runtime warning | 10m | **DONE** (v0.6.0) |
...
| 7.4 | `src/lib/tauri.ts` | Cache Tauri API module imports for performance | 10m | **DONE** (v0.7.3) |
...
| 8.3 | `src/lib/scrubber.ts` | Align hash algorithm with backend (SHA-256) | 15m | **DONE** (v0.8.2) |
