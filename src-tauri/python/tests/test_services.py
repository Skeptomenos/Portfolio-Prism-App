...
class TestDecomposer:
    ...
    def test_decompose_cache_hit(self, setup_decomposer):
        ...
        assert not errors
        cache.get.assert_called_with(isin)
...
class TestEnricher:
    ...
    def test_enrich_success(self, setup_enricher):
        ...
        service.get_metadata_batch.return_value = {
            "Stock1": {"sector": "Tech", "geography": "US", "asset_class": "Stock", "name": "Apple"},
            "Stock2": {"sector": "Tech", "geography": "US", "asset_class": "Stock", "name": "Microsoft"}
        }
...
class TestAggregator:
    ...
    def test_aggregate_simple(self, aggregator):
        direct = pd.DataFrame([{
            "ISIN": "Direct1", "Name": "Direct Stock", "NetValue": 100,
            "sector": "Tech", "geography": "US"
        }])
        
        etf = pd.DataFrame([{
            "ISIN": "ETF1", "Name": "Test ETF", "NetValue": 200
        }])
        
        # ETF holdings - 100% weight on one stock
        holdings_map = {
            "ETF1": pd.DataFrame([{
                "ISIN": "Underlying1", "Name": "Underlying",
                "Weight": 100.0,
                "sector": "Energy", "geography": "UK"
            }])
        }
