...
class TestPipelineVerification:
    ...
    def test_aggregation_math(self, mock_decomposer, mock_enricher):
        ...
        enrichment_map["US0378331005"] = pd.DataFrame([{
            "isin": "US0378331005", "name": "Apple Inc", "weight": 100.0, 
            "sector": "Technology", "geography": "United States"
        }])
        
        # 3. Aggregate
        exposure_df, errors = aggregator.aggregate(direct_pos, etf_pos, enrichment_map)
    ...
    def test_full_pipeline_orchestration(self, monkeypatch, mock_decomposer, mock_enricher, tmp_path):
