...
class TestHeadlessIntegration:
    ...
    def test_sync_portfolio_triggers_pipeline(
        ...
        with patch("portfolio_src.data.tr_sync.TRDataFetcher") as mock_fetcher_cls:
            ...
            # Note: We need to import the handler dynamically or ensure it uses the patched modules
            # prism_headless.handle_sync_portfolio does local imports, so `patch` works if it patches
            # the modules in sys.modules or where they are looked up.
            
            response = prism_headless.handle_sync_portfolio(123, payload)
