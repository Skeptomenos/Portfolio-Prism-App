...
class Pipeline:
    ...
    def run(
        ...
        if progress_callback is None:
            progress_callback = lambda msg, pct: logger.info(f"[{pct*100:.0f}%] {msg}")
        ...
        try:
            ...
            try:
                # Write health report
                self._write_health_report(errors, direct_positions, etf_positions, holdings_map)
    ...
    def _write_health_report(self, errors, direct_positions, etf_positions, holdings_map):
        ...
        for isin, holdings in holdings_map.items():
            ...
            weight_sum = float(holdings[weight_col].sum()) if weight_col and not holdings.empty else 0.0
        ...
        health_data = {
            ...
            "metrics": {
                ...
                "etfs_processed": len([e for e in etf_stats if e["status"] == "complete"]),
                "tier1_resolved": 0, 
