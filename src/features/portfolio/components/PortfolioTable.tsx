...
export const PortfolioTable = ({
  ...
  const handleCellClick = useCallback(
    (rowIndex: number, columnId: string, currentValue: string | number) => {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('isin', {
        header: 'ISIN',
        cell: (info) => (
          <span style={{ fontFamily: 'monospace', fontSize: '11px' }}>{info.getValue()}</span>
        ),
        size: 120,
      }),
      columnHelper.accessor('name', {
        header: 'Name',
        cell: (info) => (
          <EditableCell rowIndex={info.row.index} columnId="name" value={info.getValue()} />
        ),
        size: 200,
      }),
      ...
      columnHelper.accessor('instrumentType', {
        header: 'Type',
        cell: (info) => <span style={styles.type}>{info.getValue()}</span>,
        size: 80,
      }),
      ...
      columnHelper.accessor('notes', {
        header: 'Notes',
        cell: (info) => (
          <EditableCell rowIndex={info.row.index} columnId="notes" value={info.getValue() || ''} />
        ),
        size: 150,
      }),
    ],
