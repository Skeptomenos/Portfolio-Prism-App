...
import { UnresolvedItem } from '@/features/xray/hooks/usePipelineProgress'
...
export default function UnresolvedIsinsList({
  ...
  isTruncated,
  skippedTier2Count,
}: UnresolvedIsinsListProps): JSX.Element | null {
  ...
  return (
    <div className="unresolved-section">
      ...
      <div className="unresolved-header">
        ...
        {skippedTier2Count > 0 && (
          <span className="skipped-note">({skippedTier2Count} low-priority items skipped)</span>
        )}
      </div>
      ...
      <div className="unresolved-footer">
        ...
        {additionalNotShown > 0 && (
          <span className="truncated-note">{additionalNotShown} additional failures not shown</span>
        )}
      </div>
    </div>
  )
