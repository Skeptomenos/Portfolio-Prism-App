...
const Quote = Schema.Struct({ isin: Schema.String, venue: nullableText, price: scalar, time: Schema.NullOr(Schema.Number), failed: Schema.Boolean })
const Instrument = Schema.Struct({ isin: Schema.String, confirmedIsin: nullableText, priceFactor: scalar,
...
export function valuationSource(o: FinancialObservation): DataSource {
  let payload: Json[]
  if (o.sourceId === 'quotes') payload = o.quotes.map(q => ({ isin: q.isin, venue: q.venue, quote: { bid: { price: q.price, time: q.time } }, ...(q.failed ? { error: true } : {}) }))
  else if (o.sourceId === 'instrumentDetails') payload = o.instruments.map(i => ({ isin: i.isin, response: { isin: i.confirmedIsin, priceFactor: i.priceFactor, listings: i.listings.map(l => ({ slug: l.venue, currencyId: l.currency, active: l.active })) }, ...(i.failed ? { error: true } : {}) }))
