...
### Valuation investigations and manual evidence

The portfolio and held-security detail expose **Investigations**: keep a valuation gap open, exclude it from further investigation, or provide a note or dated per-security price. Exclusion is reversible and scoped to connection/account/security. It does not remove the position, substitute zero or improve financial coverage. Normal holdings sync continues; quote/instrument investigation requests are skipped only when every held account for that security in the connection is excluded. Other accounts can still receive shared compatible observations.

`GET /api/investigations` reports decisions, retained evidence and separate open/excluded/manual-supported counts. Same-origin `POST /api/investigations` accepts validated `decide`, `note`, `price` and `revoke` commands; the browser uses this core boundary. Notes never change financial values. A price requires an exact positive decimal, corroborated listing currency, ISO valuation timestamp, source and reason, and a confirmed stock/fund per-security unit. Core binds the current account and quantity-continuity boundary. It rejects future dates, dates before that boundary, unsupported crypto units and cross-account inputs. Inactive listing metadata can corroborate units/currency but never supplies the manual market price.

Eligible broker valuation always wins. Otherwise, the latest non-revoked compatible manual revision contributes immediately to valuation and exposure, with a manual label and evidence ID/source/reason/date. Changed quantities or unverified continuity make the saved entry inactive with a visible reason. Prices older than24hours remain eligible under existing policy and are visibly stale. Revoke does not resurrect a superseded price. This is user-provided evidence, not broker verification or reconciliation. See the [investigation contract](../docs/investigation-decisions.md).

Schema14 appends decisions and manual revisions/revocations. Migration creates an adjacent SQLite-consistent `.pre-history-*.sqlite` backup. Restore that untouched backup into a separate directory for recovery; never open schema14 with an older writer. Commands and their new `manual-evidence` history checkpoint commit atomically. New checkpoints pin the manual journal and valuation policy3; old policy1/2 records replay unchanged. Original broker inputs remain separate.

Checks: `pnpm --dir v2 exec vitest run tests/investigations.test.ts tests/manual-valuation.test.ts tests/investigation-http.test.ts tests/quote-acquisition.test.ts`; `pnpm --dir v2 exec tsx tests/investigations-errors-smoke.ts` checks persistent save errors and delayed-response ordering; `pnpm --dir v2 exec tsx tests/investigations-view-smoke.ts` exercises synthetic browser actions against an isolated Vite server started with `pnpm --dir v2 exec vite --host 127.0.0.1 --port 4353 --strictPort`. `pnpm --dir v2 exec tsx tests/investigations-replay.ts test-results/PRIVATE_COPY/portfolio.sqlite NEW_PRIVATE_REPORT.json` deliberately exercises only a disposable copied dataset. It reuses a retained broker bid as explicitly labelled acceptance evidence, not a new market claim; no primary, broker or credential access is performed by the runner.
...
The broker-only increment introduced valuation/calculator policy version2; manual-evidence checkpoints now use valuation policy3 with calculator2. Version 1 retains its exact eligibility, diagnostic strings and allocation output for historical replay. The expanded observation decoder accepts old records without crypto units or quote currency; only explicitly qualified new crypto observations can be priced. That broker-only increment needed no database schema change; manual evidence uses schema14 above.
...
The `multi-etf-smoke.mjs` script covers the current six-source fixture. For the current NQSE increment, run `PRISM_V2_URL=http://127.0.0.1:4330 pnpm --dir v2 exec node tests/nqse-allocation-smoke.mjs AUDIT.json` against the same private offline copied fixture; it verifies selected NQSE contributions, residual/source/date visibility, Breakdown, restart-backed totals, responsive layout and no mutations. Both scripts perform no source or broker writes. Synthetic parser/store tests require no private files. Vite's development WebSocket shares the selected HTTP server so parallel previews do not compete for a default port.
...
## Broker refresh outcome hardening

Holdings commits, valuation acquisition and transaction extraction have separate outcomes. Valuation acquisition requires instrument details, quotes and cash; optional spending balance (`availableCash`) does not gate it. Transaction failures, pending details and gaps remain visible independently. This does not claim financial completeness: excluded or unpriced positions remain unknown.

Quote routing quarantines ambiguous venues before ranking the remaining unique listings. Multiple XLON currency listings cannot block a valid unique LSX/TDG route, and ambiguous venues are never queried. Excluded positions retain their previous observation date and payload but do not inherit an earlier attempt's error.

Data & connections distinguishes the latest holdings commit from the last complete broker refresh. The latter is persisted in the existing settings table and survives diagnostic retention; upgrades seed it only from a retained proven success. A missing milestone means unknown. No history/schema migration is required. Latest transaction outcomes include standalone backfill attempts. Issuer refresh outcomes remain separate.

Safe diagnostics can retain allowlisted error subtype, connect/response phase, numeric WebSocket close code, request/operation timeout origin and per-item ISIN/venue/duration. They exclude raw reasons, payloads and credentials. An operation deadline is a failure with timeout origin, while user cancellation remains cancellation.

The saved-data Development, ETF clarity, multi-ETF, header and history browser smokes now target the current six-selected-source fixture (seven held funds; Amundi inspection-only). They retain exact contribution arithmetic and responsive/navigation checks. Run them against a private offline copied database. For unchanged financial/history acceptance, run `pnpm --dir v2 exec tsx tests/financial-equality.ts BASELINE_ORIGIN CANDIDATE_ORIGIN --broker-hardening`; this allows only the added operational `coverage.refreshIssues` field and the existing generated request clock. Replay all retained checkpoints across reopen with `pnpm --dir v2 exec tsx tests/broker-hardening-replay.ts DISPOSABLE_COPY.sqlite NEW_PRIVATE_REPORT.json`.
