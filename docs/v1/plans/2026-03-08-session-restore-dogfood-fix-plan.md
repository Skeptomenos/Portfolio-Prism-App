...
Treat this as a debugging-first auth/runtime bug, not a blind UI patch. The Trade Republic spec (`docs/v1/specs/trade_republic.md`) requires that status checks stay cached and real restore attempts only happen during explicit session-restore flows.
