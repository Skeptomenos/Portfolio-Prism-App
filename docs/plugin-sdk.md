...
| Broker | `broker-connector/1`, normalized `broker-observation/1` | `create({connectionId,vault})`; declared auth fields, cancellation, complete/partial/authoritative-empty holdings and typed observations. See `server/broker.ts`; `tests/synthetic-broker.ts` is a redistributable synthetic ledger example |
...
| Views | `view/1` | Explicit `BrowserViewModule` with a scoped client and shell presentation/route parameters. Read models: `portfolio-financial/1`, `portfolio-history/1`, bundled `wiki-pages/1`. See [financial views](financial-view-contract.md) |
