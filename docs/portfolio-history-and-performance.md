...
## Historical reconstruction and first-use onboarding

Target extension adopted 2026-09-22; not implemented. First sync saves current API observations immediately. Bounded API backfill then retrieves available past events; reviewed broker documents supplement missing periods or fields. Do not assume the API is current-only or that a PDF contains the entire account history. Establish completeness for a declared account and date range using source evidence and reconciliation.

Use the same core SQLite ledger and checkpoint system. Retain immutable API/document evidence separately from normalized economic events and calculated projections. An importer adapter parses source-specific documents into typed candidates through the broker/history boundaries; core owns acceptance, security/account identity, event deduplication, conflicts and financial meaning. No independent financial database or direct plugin writes to core tables. Exact contract changes follow inspection of representative documents.

The import journey is select files → identify account/document period → preview extracted events and gaps → review conflicts → accept supported evidence → reconcile → inspect historical results. Store document hash, page reference, original units/precision, parser version and effective dates. Repeated files and overlapping statements must not double-book events already acquired by API. Preserve independent same-value trades; ambiguous cross-source matches require review. OCR or model-assisted extraction can propose candidates, but cannot silently authorize financial postings. Process private documents locally by default; external processing needs explicit approval. Samples are required before promising supported formats or extraction accuracy.

Corrections and reversals append evidence/selection revisions. Rejecting or revoking an import selection preserves the audit trail and previously frozen checkpoints. Historical documents cannot overwrite the latest broker holdings. Continue incremental API acquisition after onboarding; use one ongoing reconciliation policy for overlapping API and document evidence.

Transactions reconstruct quantities and cash only to the extent that opening balances, transfers and corporate actions are known. A daily value series additionally needs compatible historical prices and FX, with explicit handling of split/dividend adjustments, delistings and missing dates. Trade execution prices alone cannot supply daily portfolio value. Historical company exposure additionally needs dated ETF compositions and identity relationships; today's weights cannot stand in for historical weights.

Keep **observed then**, **reconstructed from later evidence**, and unavailable periods distinct. Reconstructed projections carry source dates, evidence coverage and calculation versions and never rewrite as-recorded checkpoints. Coverage changes must not masquerade as market gains. A useful account/security history can ship before full historical company look-through.

Value-versus-net-capital charts use the scope definitions below. Purchases funded from account cash are not fresh external investment. To examine a purchase or sale in hindsight, declare horizon, benchmark, currency, costs and lot method; label any counterfactual separately. A price rise after sale does not by itself establish a bad decision. H3/H4 reconciliation gates still apply to gain and return claims; charts of already saved values do not wait for full reconstruction.
