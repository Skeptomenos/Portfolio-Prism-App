...
> **Status:** Proposed — supersedes the "Next Recommended Execution Order" in `docs/execution/stabilization-and-self-dogfood-plan.md` where they conflict.
...
| 2.7 | M | `get_engine_health` parity: Rust passes `sessionId`/`uptime` through; remove or implement `get_overlap_analysis`; rewrite `docs/specs/ipc_api.md` from the actual registries. |
...
- Update `docs/execution/stabilization-and-self-dogfood-plan.md` to point at this roadmap; keep it as the dogfood-suite tracker.
