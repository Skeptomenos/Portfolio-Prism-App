...
| P-24 | Pipeline takes 11min despite cached data | **High** | Finnhub removed (was 8min). Local batch cache added (marginal). Remaining: per-holding Hive network resolution for ~1000 tickers not in local cache. Need batch Hive lookup for <60s. Pipeline: 12min→10min→11min. | **partially fixed** (Hive batch pending) |
