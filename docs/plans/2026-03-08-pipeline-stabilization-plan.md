...
| P-24 | Pipeline takes 10min despite cached data | **High** | Finnhub fallback removed (was 8min bottleneck). Remaining: sequential ISIN resolution ~10min. Batch resolution needed for <60s target. | **partially fixed** (Finnhub removed, batch resolution pending) |
