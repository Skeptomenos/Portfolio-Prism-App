...
| P-24 | Pipeline takes 10min on repeated runs despite cached data | **High** | **Root cause found:** Only 52% local cache hit rate (748/1426 tickers). 678 tickers NOT in cache despite multiple runs. Three causes: (1) Hive sync caps at 1000 listings, (2) per-holding resolver caches under variant ticker not original, (3) `upsert_listing` only runs for batch Hive hits, not for slow-path resolutions. Fix: after any successful resolution, always `upsert_listing(original_ticker, ...)`. Second run should be <30s. | **root cause found** |
