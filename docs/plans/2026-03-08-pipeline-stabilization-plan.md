...
| P-24 | Pipeline takes 10min on repeated runs | **High** | **Three root causes:** (1) Supabase RPC capped at 1000 rows (PostgREST default) — Hive has more data but sync only gets 1000. Not our code, Supabase config. (2) Local cache 52% hit rate (748/1426) — resolved ISINs not cached under all ticker variants. (3) 678 uncached tickers go through sequential RPCs. **Fix:** (a) Increase Supabase row limit or paginate, (b) cache ALL ticker variants after resolution (original + generated), (c) target <30s on second run. | **root cause found** |
