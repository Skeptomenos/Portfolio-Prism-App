...
### 1.4 Visual Presentation (Beautiful Logs)
Runtime logs must be visually structured to be readable at a glance:
- **Prefix:** Every application log line must start with a consistent prefix: `  PRISM ↳ `.
- **Color Coding:** Use ANSI escape codes for log levels:
    - `INFO`: Blue (`\x1b[34m`)
    - `WARN`: Yellow (`\x1b[33m`)
    - `ERROR`/`FATAL`: Red (`\x1b[31m`)
    - `LOG`/`DEBUG`: Gray (`\x1b[90m`)
- **Indentation:** Use consistent indentation to separate application logs from system/build logs.
- **Density:** Prefer single-line summaries for repetitive tasks (e.g., batch processing) over multi-line floods.
