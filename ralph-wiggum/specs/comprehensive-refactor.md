# Comprehensive Refactoring Plan (Verified Remaining Work V5)
...
- [ ] 2. **Move Feature Hooks & Store (FSD)**
  ...
  - [ ] **Action**: Create `src/features/auth/store/authSlice.ts` (Zustand slice) and refactor `src/store/useAppStore.ts` to compose slices.
...
- [ ] 3. **Refactor Internal Mocking (Frontend)**
  ...
  - [ ] **Action**: Create `src/test/mocks/tauri.ts` using **Window Stub Pattern**:
    ...
    ```
  - [ ] **Action**: Refactor `usePortfolioData.test.tsx` and `LoginForm.test.tsx` to use `setupTauriMock()` instead of `vi.mock('../ipc')`.
...
- [ ] 5. **Frontend Type Safety**
  ...
  - [ ] **Action**: Define `TauriFile` interface in `src/types/tauri.ts`:
    ```typescript
    export interface TauriFile { path: string; name: string; size: number; }
    ```
  - [ ] **Action**: Remove `any` from `src/features/integrations/components/HoldingsUpload.tsx` using `TauriFile`.
  - [ ] **Action**: Add `: JSX.Element` return type to all components in `src/features/`.
...
- [ ] 7. **Python Structured Logging & MD5 Replacement**
  ...
  - [ ] **Action**: Refactor `logger.info(f"...")` to `logger.info("...", extra={...})` in ALL modules.
